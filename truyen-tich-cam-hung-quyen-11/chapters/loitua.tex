...
Bộ sách <<Truyện Tích Cảm Hứng Song Ngữ>> gồm 28 quyển, mỗi quyển một chủ đề riêng. Quyển XI này mang chủ đề: \textbf{tự mãn, cái tôi, so sánh, trì hoãn}.
\textit{(The series ``Inspiring Bilingual Tales'' contains 28 volumes, each with its own theme. Volume XI focuses on: \textbf{tự mãn, cái tôi, so sánh, trì hoãn}.})
\end{truyen}
